...
from visual_ai_studio.domain.validators import sha256_file, validate_artifact_package
...
def test_invalid_image_is_detected(
    ...
    report = validate_artifact_package(uuid4(), paths)
...
def test_multiple_images_are_allowed(
    ...
    Image.new("RGB", (640, 640), "white").save(second, format="WEBP")
    paths.append(second)
...
def test_supported_legacy_artifact_values_are_readable() -> None:
