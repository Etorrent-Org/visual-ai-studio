...
class SubmissionPage(QWidget):
    ...
    def __init__(self) -> None:
        ...
        subtitle = QLabel(
            "Exportez le résultat localement ou utilisez "
            "un webhook facultatif."
        )
    ...
    def update_state(
        ...
        if project is None:
            ...
            self.validation.setText(
                "Validez d'abord un résultat."
            )
            return

        self.summary.setText(
            f"{project.title} • version {project.version}"
        )
        ...
        if ready:
            ...
            self.validation.setText(
                "Projet validé : export local disponible."
            )

        if ready and configured:
            self.submit_button.setEnabled(True)
            self.validation.setText(
                "Projet validé : export local ou webhook disponible."
            )
    ...
    def set_outcome(
        ...
        if outcome.execution_id:
            details.append(
                f"Exécution distante : {outcome.execution_id}"
            )

        if outcome.remote_url:
            details.append(
                f"URL distante : {outcome.remote_url}"
            )

        if outcome.unknown:
            details.append(
                "Le statut distant est inconnu."
            )
