...
class DashboardPage(QWidget):
    ...
    def __init__(
        ...
        subtitle = QLabel("Brief → Studio Visuel → validation → export")
        ...
        self.status_filter.setMaximumWidth(190)

        self.status_filter.setFixedHeight(36)

        self.status_filter.setMaxVisibleItems(6)
        ...
        self.status_filter.view().setMaximumHeight(150)
        ...
        self.new_button.setMinimumWidth(138)
        ...
        list_title = QLabel("Liste des projets")
        ...
        self.search.setPlaceholderText("Rechercher par nom, collection ou style…")
        ...
        header_view.setMinimumHeight(40)
        ...
        panel_layout.addLayout(list_header)
    ...
    def _update_metrics(
        ...
        brief_count = sum(1 for project in self._projects if project.status is ProjectStatus.BRIEF)

        validated_count = sum(
            1 for project in self._projects if project.status is ProjectStatus.VALIDATED
        )

        archived_count = sum(
            1 for project in self._projects if project.status is ProjectStatus.ARCHIVED
        )

        self.total_metric.set_value(total)

        self.brief_metric.set_value(brief_count)

        self.validated_metric.set_value(validated_count)

        self.archived_metric.set_value(archived_count)
    ...
    def _apply_filters(
        ...
        for project in self._projects:
            status_matches = not status or project.status.value == status
            ...
            text_matches = not needle or needle in searchable
        ...
        status_colors = {
            ProjectStatus.BRIEF.value: ("#60A5FA"),
            ProjectStatus.VALIDATED.value: ("#4ADE80"),
            ProjectStatus.ARCHIVED.value: ("#94A3B8"),
        }
        ...
        for row, project in enumerate(visible):
            values = [
                ...
                project.updated_at.astimezone().strftime("%d/%m/%Y %H:%M"),
            ]
    ...
    def _update_actions(
        ...
        selected = bool(self._selected_id())
